...
class CalendarCommands(commands.Cog):
    ...
    @commands.command(name="simplecalendar", aliases=["scalendar"])
    @rate_limit(calls=2, period=60, key_func=guild_key)  # 2 calls per minute per guild
    async def simple_calendar(self, ctx, month: Optional[int] = None, year: Optional[int] = None):
        """
        Display a calendar showing all XP boost events (simplified version)
        
        Usage:
          !!simplecalendar           - Shows current month
          !!simplecalendar 12        - Shows December of current year
          !!simplecalendar 3 2024    - Shows March 2024
        """
        guild_id = str(ctx.guild.id)
        ...
        try:
            ...
            # Generate the calendar image using the minimal implementation
            image_bytes = await generate_minimal_calendar(
                guild_name=ctx.guild.name,
                year=year,
                month=month,
                events=all_events,
            ...
            embed.add_field(
                ...
                value=(
                    "**View another month:**\n"
                    f"`!!simplecalendar {month-1 if month > 1 else 12} {year if month > 1 else year-1}` - Previous Month\n"
                    f"`!!simplecalendar {month+1 if month < 12 else 1} {year if month < 12 else year+1}` - Next Month"
                ),
                inline=False
            )
            ...
            logging.error(f"Error in simple_calendar command: {e}", exc_info=True)
            await loading_msg.edit(content=f"❌ Error generating calendar: {str(e)}")
